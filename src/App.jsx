...
const BRAND = { name: "RFM", pro: "Pro", color: "#6366f1" };
...
function buildClients(rows,k=5){
  ...
  const churnMap={"#8b5cf6":0.05,"#10b981":0.15,"#f59e0b":0.55,"#ef4444":0.80,"#3b82f6":0.30};
  return arr.map((c,i)=>{const rs=sc(c.rn_n),fs=sc(c.fn_n),ms=sc(c.mn2_n),seg=sg(rs,fs,ms);const base=churnMap[SEGS[seg].hex]||0.3;return{...c,cluster:asgn[i],rs,fs,ms,segment:seg,churn:Math.min(1,Math.max(0,base+(Math.random()-.5)*.08)).toFixed(2)};});
}
...
function MiniBar({value,max,color}){
  return <div style={{flex:1,height:6,borderRadius:3,background:"#f1f5f9",overflow:"hidden"}}>
...
export default function App(){
  ...
  const handleFile=e=>{
    ...
    r.onload=ev=>{try{setClients(buildClients(parseCSV(ev.target.result)));setTpage(0);}catch{alert("Erro ao processar CSV.");}setLoading(false);};
    r.readAsText(f);
  };
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"system-ui,sans-serif",background:"#f8fafc",color:"#0f172a",fontSize:14}}>
      ...
      <div style={{width:sideOpen?220:64,flexShrink:0,background:"#1e1b4b",display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden"}}>
        ...
        <div style={{flex:1,padding:"12px 0"}}>
          ...
          {navItems.map(item=>{
            ...
            return <div key={item.id} onClick={()=>setPage(item.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",cursor:"pointer",borderRadius:8,margin:"2px 8px",background:act?"rgba(99,102,241,0.2)":"transparent",color:act?"#a5b4fc":"rgba(255,255,255,0.6)",transition:"all .15s",whiteSpace:"nowrap"}}>
        ...
        <div style={{padding:12,borderTop:"1px solid rgba(255,255,255,0.08)"}}>
          <div onClick={()=>setSideOpen(!sideOpen)} style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"10px 16px",cursor:"pointer",borderRadius:8,margin:"2px 8px",color:"rgba(255,255,255,0.6)"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* Header */}
        <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"0 24px",height:60,display:"flex",alignItems:"center",gap:16,flexShrink:0}}>
          <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {["Todos",...SK].map(sg=>{
              ...
              return <button key={sg} onClick={()=>{setFilter(sg);setTpage(0);}} style={{padding:"5px 12px",borderRadius:20,border:act?`1.5px solid ${col||"#6366f1"}`:"1px solid #e2e8f0",background:act?(SEGS[sg]?.bg||"#eff6ff"):"#fff",color:act?(SEGS[sg]?.tx||"#1e3a8a"):"#64748b",fontSize:11,fontWeight:act?600:400,cursor:"pointer"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          ...
          {page==="overview"&&<div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
              {SK.map(sg=>{
                ...
                return(
                  <div key={sg} onClick={()=>setFilter(filter===sg?"Todos":sg)} style={{background:filter===sg?SEGS[sg].bg:"#fff",borderRadius:12,border:`${filter===sg?"1.5px":"1px"} solid ${filter===sg?SEGS[sg].hex+"66":"#e2e8f0"}`,padding:"14px 16px",cursor:"pointer",transition:"all .15s"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div style={{background:"#fff",borderRadius:14,border:"1px solid #e2e8f0",padding:"20px 22px"}}>
                ...
                {SK.map(sg=>{
                  const churnVal={"#8b5cf6":0.05,"#10b981":0.15,"#f59e0b":0.55,"#ef4444":0.80,"#3b82f6":0.30}[SEGS[sg].hex]||0.3;
                  return(
          ...
          {page==="scatter"&&<div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {hov&&(
              <div style={{background:"#fff",borderRadius:14,border:`1.5px solid ${SEGS[hov.segment].hex}44`,padding:"16px 20px",display:"flex",gap:20,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  {[["R",hov.rs],["F",hov.fs],["M",hov.ms]].map(([l,v])=>(
                    <div key={l} style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div style={{display:"flex",gap:3}}>
                        {[1,2,3,4,5].map(i=><div key={i} style={{width:14,height:5,borderRadius:2,background:i<=v?(v>=4?"#10b981":v>=3?"#f59e0b":"#ef4444"):"#e2e8f0"}}/>)}
                      </div>
          ...
          {/* Clients Table */}
          {page==="clients"&&<div style={{background:"#fff",borderRadius:14,border:"1px solid #e2e8f0",padding:"20px 22px"}}>
                ...
                <tbody>
                  {pageD.map((c,i)=>(
                    <tr key={c.id} style={{background:i%2?"#fafafa":"#fff",borderBottom:"1px solid #f1f5f9"}}>
                      ...
                      {[c.rs,c.fs,c.ms].map((v,j)=>(
                        <td key={j} style={{padding:"9px 12px"}}>
                          <div style={{display:"flex",gap:2}}>{[1,2,3,4,5].map(i=><div key={i} style={{width:12,height:4,borderRadius:2,background:i<=v?(v>=4?"#10b981":v>=3?"#f59e0b":"#ef4444"):"#e2e8f0"}}/>)}</div>
                        </td>
                      ))}
                    </tr>
                  ))}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:16,fontSize:12,color:"#64748b"}}>
              <span>Página {tpage+1} de {tPages||1}</span>
          ...
          {page==="top"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12}}>
              {[...clients].sort((a,b)=>b.mon-a.mon).slice(0,3).map((c,i)=>(
                <div key={c.id} style={{background:"#fff",borderRadius:14,border:"1px solid #e2e8f0",padding:"20px 22px",borderTop:`4px solid ${i===0?"#f59e0b":i===1?"#94a3b8":"#cd7c3f"}`,textAlign:"center"}}>
            ...
            <div style={{background:"#fff",borderRadius:14,border:"1px solid #e2e8f0",padding:"20px 22px"}}>
              <div style={{fontSize:13,fontWeight:700,marginBottom:16}}>Top 10 por Receita</div>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr style={{borderBottom:"2px solid #f1f5f9"}}>
